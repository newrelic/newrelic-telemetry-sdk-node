...
test('Span batch', (t):void => {
  t.test('sets properties on initialization', (t) => {
  ...
  t.test('adds attributes to common.attributes prop when provided in constructor', (t) => {
    const attributes = {
      'name': 'name',
    ...
    t.equal(batch.common.attributes["duration.ms"], 808, 'sets duration.ms attribute')
    t.equal(batch.common.attributes["service.name"], 'serviceName', 'sets service.name attribute')
